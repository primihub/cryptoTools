...
//
  ...
  if (!areDistint(cd, a2, diff) || !areDistint(cd, a1, diff))
    throw std::runtime_error("must be distinct " LOCATION);
...
//
  if (prod.mWires.size() > a.mWires.size() + b.mWires.size())
    throw std::runtime_error(LOCATION);

  if (a.size() >= b.size()) {
    u64 numRows = useTwoComplement ? prod.mWires.size() : std::min(b.size(), prod.size());
    ...
    for (u64 i = 0; i < rows.size(); ++i) {
      // this will hold the b[i] * a
      rows[i].mWires.resize(std::min<u64>(prod.mWires.size() - i, a.mWires.size()));
      ...
      if (i == 0) {
        ...
        // the running total, so we want it to be
        // the wires that represent the product c.
        rows[0].mWires[0] = prod.mWires[0];
      }

      if (rows.size() == 1) {
        for (u64 j = 1; j < rows[0].mWires.size(); ++j) {
          rows[0].mWires[j] = prod.mWires[j];
        }
      }

      if (a.mWires.size() == 1) {
          rows[i].mWires[0] = prod.mWires[i];
      }
...
#define SERIAL
    ...
    if (rows.size() > 1) {
      ...
      // bit of the final product.
      for (u64 i = 1; i < rows.size(); i++) {
        BetaBundle sum(std::min<u64>(rows[i].mWires.size() + 1, prod.mWires.size() - i));
        ...
        sum.mWires[0] = prod.mWires[i];

        if (i == rows.size() - 1) {
          for (u64 j = 1; j < sum.mWires.size(); ++j) {
            sum.mWires[j] = prod.mWires[i + j];
          }
        }
        ...
        // std::cout << "add[" << i << "] " << cd.mNonXorGateCount
        //             << "  (+" << (cd.mNonXorGateCount - prev) << ")" << std::endl;
    ...
    int_int_mult_build(cd, b, a, prod, op, useTwoComplement);
  }
}
...
//
  ...
  for (i64 i = shifts; i >= 0; --i) {
    ...
    // std::cout << "iter[" << i << "]  geq  " << cd.mNonXorGateCount
    //           << "  (+" << (cd.mNonXorGateCount - prev) << ")  "
    //           << remTemp.mWires.size() << std::endl;
...
void BetaLibrary::int_int_bitwiseOr_build(
      BetaCircuit& cd,
      const BetaBundle & a1,
      const BetaBundle & a2,
      const BetaBundle & out) {
...
void BetaLibrary::int_int_bitwiseXor_build(
      BetaCircuit& cd,
      const BetaBundle & a1,
      const BetaBundle & a2,
      const BetaBundle & out) {
